...
$(function(){
   ...
   interval = setInterval(function(){
        if(count < 10){
            ...
            // 클래스가 area인 요소 내부에 있는 내용을 모두삭제
            $(".area").empty();

            // count를 0으로 초기화하여
            // 다시 0~9까지 카운트가 가능하도록함
            count = 0;
        }
    },300);

    // HTML 문서의 로딩이 완료된 후
    //아이디가 stop인 요소가 클릭 되었을 때 동작을 추가
    $("#stop").on("click",function(){
        clearInterval(interval);
    });

    // 클래스가 text1인 요소를 클릭했을 때
    // 콘솔에 요소의 내용(text)을 출력
    ...
    /* 
        ...
        2. on(), addEventListener()는
            ...
            추가하는 메소드이다.
        (중요) 기존에 on(), addEventLitener()를 이용하여
        이벤트 동작을 추가할 때는
...
$("#change1").on("change",function(){
    console.log("chechbox 값이 변경되었습니다.");
})

// 아이디가 change2인 요소의 입력값 이 변한 상태로 포커스가 해제될 때마다
// 콘솔에 "입력값이 변경되었습니다." 출력
...
$("#input-content").on("input",function(){
    ...
    if(length <= 130){
        ...
        // $("#input-content").prop("readonly",false)
    }else if(length <= 149){
        $("#counter").css("color","orange");
        // $("#input-content").prop("readonly",false)
    }else{
        $("#counter").css("color","red");
        // $("#input-content").prop("readonly",true)
       ...
       //151번째 입력이 인식되면
       //150글자로 자르는건데 자르기 전에 글자수를 카운트해서 151로 보이는 문제발생
       $("#counter").text(150);
