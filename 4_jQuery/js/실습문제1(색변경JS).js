...
for (let i = 0; i < area.length; i++) {
    area[i].style.cssText  = 'width: 150px; height: 170px; border:1px solid black; display:flex; flex-direction:column';
...
for (let i = 0; i < box.length; i++) {
    box[i].style.cssText = ' height: 150px; border-bottom: 1px solid black;';
...
for (let i = 0; i < boxColor.length; i++) {
    ...
    document.querySelectorAll("input[class = 'box-color']")[i].addEventListener("keyup",function(){
...
document.getElementById("btn1").addEventListener("click",function(){
   ...
   print1 = document.getElementById("print1");
   for (let i = 0; i < box.length; i++) {
    box[i].style.cssText += 'transition-duration : '+inp1.value+'s;';
    print1.innerText = inp1.value;
   }
})
...
document.getElementById("clearBtn").addEventListener("click",function(){
    for (let i = 0; i < box.length; i++) {
        ...
        box[i].nextElementSibling.value = "";
        box[i].style.transition = '0s';
        inp1.value = "";
        print1.innerText = 0;
       }
})
...
//     for(let i=0; i<box.length; i++){
//         box[i].style.backgroundColor = "";  // .box 배경색 삭제
//         boxColor[i].value = ""; // .box-color 값 삭제
//     }
// })